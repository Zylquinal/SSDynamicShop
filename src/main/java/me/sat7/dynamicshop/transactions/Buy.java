...
public final class Buy {
    ...
    public static void buyItemCash(Player player, String shopName, String tradeIdx, ItemStack tempIS, double priceSum, double deliverycharge, boolean infiniteStock)
    {
        ...
        if(actualAmount <= 0)
        {
            player.sendMessage(DynamicShop.dsPrefix + Objects.requireNonNull(LangUtil.ccLang.get().getString("NOT_ENOUGH_MONEY")).replace("{bal}",econ.format(econ.getBalance(player))));
        ...
        if(econ.getBalance(player) >= priceSum)
        {
            ...
            {
                leftAmount(player, shopName, tradeIdx, tempIS, actualAmount);

                //로그 기록
                LogUtil.addLog(shopName,tempIS.getType().toString(),actualAmount,priceSum,"vault",player.getName());

                player.sendMessage(DynamicShop.dsPrefix + Objects.requireNonNull(LangUtil.ccLang.get().getString("BUY_SUCCESS"))
    ...
    private static void leftAmount(Player player, String shopName, String tradeIdx, ItemStack tempIS, int actualAmount) {
    ...
    public static void buyItemJobPoint(Player player, String shopName, String tradeIdx, ItemStack tempIS, double priceSum, double deliverycharge, boolean infiniteStock)
    {
        ...
        if(actualAmount <= 0)
        {
            player.sendMessage(DynamicShop.dsPrefix+ Objects.requireNonNull(LangUtil.ccLang.get().getString("NOT_ENOUGH_POINT")).replace("{bal}", DynaShopAPI.df.format(JobsHook.getCurJobPoints(player))));
            ShopUtil.ccShop.get().set(shopName+"." + tradeIdx + ".stock", stockOld);
            return;
        }
        ...
        if(JobsHook.getCurJobPoints(player) >= priceSum)
        {
            if(JobsHook.addJobsPoint(player,priceSum * -1))
            {
                ...
                player.sendMessage(DynamicShop.dsPrefix + Objects.requireNonNull(LangUtil.ccLang.get().getString("BUY_SUCCESS_JP"))
                        .replace("{item}", ItemsUtil.getBeautifiedName(tempIS.getType()))
                        .replace("{amount}", Integer.toString(actualAmount))
