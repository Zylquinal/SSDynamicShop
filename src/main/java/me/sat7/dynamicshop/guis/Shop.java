...
public class Shop {
    ...
    public Inventory getGui(Player player, String shopName, int page) {
        ...
        ItemStack pageBtn =  ItemsUtil.createItemStack(Material.PAPER,null,
                page + "/" + Objects.requireNonNull(ShopUtil.ccShop.get().getConfigurationSection(shopName).getConfigurationSection("Options")).getInt("page") + " " + LangUtil.ccLang.get().getString("PAGE"),
                pageLore,page);
        ...
        if(ShopUtil.ccShop.get().contains(shopName+".Options.lore"))
        {
            ...
            if(Objects.requireNonNull(loreTxt).length() > 0)
            {
        ...
        if(!(Objects.requireNonNull(perm).length()==0)) {
        ...
        // 플래그
        if(ShopUtil.ccShop.get().contains(shopName+".Options.flag") && Objects.requireNonNull(ShopUtil.ccShop.get().getConfigurationSection(shopName + ".Options.flag")).getKeys(false).size() > 0)
        {
            infoLore.add(LangUtil.ccLang.get().getString("FLAG")+":");
            for (String s: Objects.requireNonNull(ShopUtil.ccShop.get().getConfigurationSection(shopName + ".Options.flag")).getKeys(false))
            {
                infoLore.add("§7 - "+s);
            }
        }
        ...
        {
            String[] temp = Objects.requireNonNull(ShopUtil.ccShop.get().getString(shopName + ".Options.shophours")).split("~");
        ...
        for (String s: Objects.requireNonNull(ShopUtil.ccShop.get().getConfigurationSection(shopName)).getKeys(false))
        {
            try
            {
                ...
                ItemStack itemStack = new ItemStack(Objects.requireNonNull(Material.getMaterial(Objects.requireNonNull(itemName))),1); // 아이탬 생성
                ...
                if(ShopUtil.ccShop.get().contains(shopName+"."+s+".value"))
                {
                    ...
                    if(!Objects.requireNonNull(tradeType).equalsIgnoreCase("SellOnly")) lore.add(LangUtil.ccLang.get().getString("PRICE") + df.format(buyPrice) + " " + valueChangedRange);
                    ...
                    if(Objects.requireNonNull(LangUtil.ccLang.get().getString("TRADE_LORE")).length() > 0) lore.add(LangUtil.ccLang.get().getString("TRADE_LORE"));
