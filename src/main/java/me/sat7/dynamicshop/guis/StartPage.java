...
public class StartPage {
    ...
    public Inventory getGui(Player player) {
        Inventory ui = Bukkit.createInventory(player, ccStartPage.get().getInt("Options.UiSlotCount"), Objects.requireNonNull(ccStartPage.get().getString("Options.Title")));
        ...
        for (String s: Objects.requireNonNull(cs).getKeys(false))
        {
            try {
                ...
                {
                    name= Objects.requireNonNull(cs.getConfigurationSection(s)).getString("displayName");
                }

                ArrayList<String> tempList = new ArrayList<>();
                if(cs.contains(s+".lore"))
                {
                    String[] lore = Objects.requireNonNull(cs.getConfigurationSection(s).getString("lore")).split(Objects.requireNonNull(ccStartPage.get().getString("Options.LineBreak")));
                    tempList.addAll(Arrays.asList(lore));
                }

                if(player.hasPermission("dshop.admin.shopedit"))
                {
                    if(Objects.requireNonNull(cs.getString(s + ".action")).length()>0)
                    {
                ...
                ItemStack btn = new ItemStack(Objects.requireNonNull(Material.getMaterial(Objects.requireNonNull(Objects.requireNonNull(cs.getConfigurationSection(s)).getString("icon")))));
