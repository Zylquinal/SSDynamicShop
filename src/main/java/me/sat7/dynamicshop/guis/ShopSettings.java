...
public class ShopSettings {
    public Inventory getGui(Player player, String shopName) {
        Inventory inventory = Bukkit.createInventory(player,36, Objects.requireNonNull(LangUtil.ccLang.get().getString("SHOP_SETTING_TITLE")));
        ...
        ConfigurationSection confSec_Options = Objects.requireNonNull(ShopUtil.ccShop.get().getConfigurationSection(shopName)).getConfigurationSection("Options");
        String permStr = Objects.requireNonNull(confSec_Options).getString("permission");
        String permNew = "dshop.user.shop."+shopName;
        Material permIcon;
        if(Objects.requireNonNull(permStr).isEmpty())
        {
        ...
        if(!Objects.requireNonNull(permStr).equalsIgnoreCase(LangUtil.ccLang.get().getString("NULL(OPEN)")))
        {
        ...
        if(ShopUtil.ccShop.get().contains(shopName+".Options.shophours"))
        {
            ...
            ItemStack open24Btn =  ItemsUtil.createItemStack(Material.CLOCK,null,
                    ...
                    new ArrayList<>(Arrays.asList(
                            Objects.requireNonNull(LangUtil.ccLang.get().getString("TIME.CUR")).replace("{time}",curTime+""),
            ...
            ItemStack taxToggleBtn =  ItemsUtil.createItemStack(Material.IRON_INGOT,null,
                    ...
                    new ArrayList<>(Arrays.asList(
                            ...
                            "§e"+ LangUtil.ccLang.get().getString("CLICK")+": " +
                                    Objects.requireNonNull(LangUtil.ccLang.get().getString("TAX.USE_GLOBAL")).replace("{tax}",globalTax+"")
                    )),1);
