...
public final class ShopUtil {
    ...
    public static int findEmptyShopSlot(String shopName)
    {
        ...
        for (String s: Objects.requireNonNull(ccShop.get().getConfigurationSection(shopName)).getKeys(false))
        {
            try {
    ...
    public static int findItemFromShop(String shopName, ItemStack item)
    {
        for (String s: Objects.requireNonNull(ccShop.get().getConfigurationSection(shopName)).getKeys(false))
        {
            ...
            if(Objects.requireNonNull(ccShop.get().getString(shopName + "." + s + ".mat")).equals(item.getType().toString()))
            {
    ...
    public static void insetShopPage(String shopName, int page)
    {
        ...
        Objects.requireNonNull(confSec).set("Options.page", confSec.getInt("Options.page")+1);
    ...
    public static void mergeShop(String shopA, String shopB)
    {
        ...
        int pg1 = Objects.requireNonNull(confA).getInt("Options.page");
        int pg2 = Objects.requireNonNull(confB).getInt("Options.page");
        ...
        if(confA.contains("Options.Balance") || confB.contains("Options.Balance"))
        {
            ...
            double b = 0;
            if(!(Objects.requireNonNull(confA.getString("Options.Balance")).equals(shopB) || Objects.requireNonNull(confB.getString("Options.Balance")).equals(shopA) ))
            {
    ...
    public static double getShopBalance(String shopName)
    {
        ...
        try
        {
            shopBal = Double.parseDouble(Objects.requireNonNull(ccShop.get().getString(shopName + ".Options.Balance"))); // 파싱에 실패하면 캐치로 가는 방식.
        }
        // 연동형
        catch (Exception ee)
        {
            ...
            // 그런 상점이 없음.
            if(!ccShop.get().contains(Objects.requireNonNull(linkedShop)))
            {
            ...
            try
            {
                ...
                {
                    double temp = Double.parseDouble(Objects.requireNonNull(ccShop.get().getString(linkedShop + ".Options.Balance")));
                }
                else
                {
                    return -1;
    ...
    public static void addShopBalance(String shopName, double amount)
    {
        ...
        try
        {
            Double temp = Double.parseDouble(Objects.requireNonNull(ccShop.get().getString(shopName + ".Options.Balance")));
    ...
    {
        if(i.getSize() == 54 && i.getItem(53) != null && Objects.requireNonNull(i.getItem(53)).getType().name().contains("SIGN"))
        {
            String temp = ChatColor.stripColor(Objects.requireNonNull(i.getItem(53)).getItemMeta().getDisplayName());
    ...
    {
        File[] allFile = new File(Objects.requireNonNull(Bukkit.getServer().getPluginManager().getPlugin("DynamicShop")).getDataFolder() + "/Convert/Shop").listFiles();

        if(Objects.requireNonNull(allFile).length == 0)
        {
        ...
        for(File f:allFile)
        {
            try
            {
                ...
                if(cc.open(filename,"Convert/Shop"))
                {
                    ...
                    String[] itemList = Objects.requireNonNull(confSec.getString("items")).split("},");
...
//                        }
                        ...
                        try
                        {
                            ...
                            ccShop.get().set(shopname+"."+ idx +".mat", Objects.requireNonNull(m).name());
                            idx += 1;
                        }catch (Exception e)
                        {
                    ...
                    idx = 0;
                    for(String s: Objects.requireNonNull(confSec.getConfigurationSection("slotdata")).getKeys(false))
                    {
    ...
    {
        Bukkit.getScheduler().runTaskLater(DynamicShop.plugin, (@NotNull Runnable) player::closeInventory,2);
    }
    ...
    public static void setupShopFile()
    {
        ...
        {
            if(!Objects.requireNonNull(ccShop.get().getConfigurationSection(s)).contains("Options"))
            {
