...
public final class ItemsUtil {
    ...
    public static void sendItemInfo(Player player, String shopName, int idx, String msgType)
    {
        ...
        player.sendMessage(" - " + Objects.requireNonNull(LangUtil.ccLang.get().getString(msgType)).
                replace("{item}", Objects.requireNonNull(ShopUtil.ccShop.get().getString(shopName + "." + idx + ".mat"))).
                replace("{info}",info)
        );
    }
}
