...
public class OpenShop {
    public static boolean openShop(String[] args, CommandSender sender) {
        ...
        if (target != null) {
            ...
            if (Objects.requireNonNull(shopConf).contains("shophours") && !target.hasPermission("dshop.admin.shopedit")) {
                ...
                if (close > open) {
                    if (!(open <= curTime && curTime < close)) {
                        target.sendMessage(DynamicShop.dsPrefix + Objects.requireNonNull(LangUtil.ccLang.get().getString("TIME.SHOP_IS_CLOSED")).
